...
async function main() {
	...
	console.log("MindMesh LLM provider test");
	...
	const extracted = await runCheck("LLM graph extraction", async () => {
		const text = args.textParts.join(" ").trim() || SAMPLE_TEXT;
		const graph = await llmProvider.extractGraph({
			text,
			systemPrompt: buildExtractionPrompt(prompts.extractionSystemTemplate, {
				graphSchema: prompts.graphSchema,
...
main().catch((error) => {
	console.error("MindMesh LLM test failed.");
